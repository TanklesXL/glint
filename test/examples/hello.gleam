...
pub fn app() {
  ...
  // with an app version of "dev", this is used when the version flag is used
  |> glint.with_version("dev")
  ...
  // add a root command
  |> glint.add(at: [], do: hello_cmd())
  // add a 'single' command
  |> glint.add(at: ["single"], do: hello_single_cmd())
  // add a 'custom'
  |> glint.add(at: ["custom"], do: hello_custom_cmd())
}
